...
TestSignalToNoise("ifp",log_transform = FALSE,arcsin_sqrt_transform = FALSE)
...
ylabel = "Production (?)"
maintitle = "Inland fisheries production"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,year)", 2, "eafs", "list(a =142, b = -0.07)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = FALSE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "First order polynomial")
...
#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,year)", 2, "eafs", "list(a = 200, b = -1)", "coef(m1)", "coef(m2)", TRUE,fitAR1 = TRUE,fitAR2 = FALSE,num_data_points = num_data_points, ylabel, "Hyperbolic")

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "coef(m2)", TRUE,fitAR1 = TRUE,fitAR2 = TRUE,  num_data_points = num_data_points, ylabel, "Michaelis-Menten",0.1)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = TRUE, fitAR2 = TRUE, num_data_points = num_data_points, ylabel, "Hollings Type IV")

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE,fitAR1 = TRUE,fitAR2 = FALSE,  num_data_points = num_data_points, ylabel, "Rational")
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW 
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m2)",  fitAR1 = TRUE, TRUE,  num_data_points = num_data_points, ylabel, "Power")
...
#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE,  fitAR2 =TRUE, num_data_points = num_data_points, ylabel, "Shepherd")

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,year)", 3, "eafs", "list(a = 28, b = -0.02, d = -0.2)", "list(a = 28, b = -0.02, d = -0.2)", "coef(m2)", TRUE,  fitAR2 =FALSE,  num_data_points = num_data_points, ylabel, "Hassell")
...
if (ARCSINSQRT)
{
  for (ii in 1:dim(modelses)[1])
  {
    for (jj in 1:dim(modelses)[2])
    {
      if (modelses[ii,jj] > pi/2 & !is.na(modelses[ii,jj]))
        modelses[ii,jj] = pi/2
    }
  }
}
...
writepdfsandresults(output_directory_images, output_directory_files, output_name, 20000)
