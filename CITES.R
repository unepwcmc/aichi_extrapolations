...
data_directory = "C:/Users/samanthah/Documents/Aichi Extrapolations/Final_data/biodbar1/"
...
TestSignalToNoise("CITES Cat 1 Parites",log_transform = FALSE,arcsin_sqrt_transform = TRUE)
# File name for the input file
Filename = "CITES Cat 1 Parites"
...
y_limit = max(back_transform(eafs[,2])) * 1.5
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,year)", 2, "eafs", "list(a =142, b = -0.07)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "First order polynomial")

# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,year)", 2, "eafs", "list(a = 20, b = 0.1)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = TRUE,fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "2 parameter negative exponential")

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,year)", 3, "eafs", "list(a = 30, b = 1, c = -0.5)", "coef(m1)", "list(a = 30, b = 1, c = -0.5)",  TRUE, fitAR1 = TRUE,fitAR2 = TRUE,num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
...
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "Michaelis-Menten")

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,year)", 2, "eafs", "list(a = 0.1, b = 1)", "coef(m1)", "coef(m2)", TRUE,fitAR1 = TRUE,fitAR2 = TRUE, num_data_points = num_data_points, ylabel, "Hollings Type III")

#--------------------------------------------------------------------------------------
# MODEL 7: HOLLING TYPE IV
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = TRUE, num_data_points = num_data_points, ylabel, "Hollings Type IV")

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE,fitAR1 = FALSE,fitAR2 = FALSE,  num_data_points = num_data_points, ylabel, "Rational")
...
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,year)", 2, "eafs", "list(a = 2, b = -0.0001)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE,fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Monomolecular")

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE,  fitAR1 = FALSE,fitAR2 = FALSE,num_data_points = num_data_points, ylabel, "Ricker")

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,year)", 3, "eafs", "list(a = 1, b = -0.1 ,d = -5)", "coef(m1)", "coef(m2)", TRUE,fitAR1 = FALSE,fitAR2 = FALSE,  num_data_points = num_data_points, ylabel, "Gompertz")

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,year)", 3, "eafs", "list(a = 2, b = -0.1,d = -0.1)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, fitAR1 = FALSE,fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Chapman-Richards")

#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,year)", 4, "eafs", "list(a = 20, b = 5,d = 0.2, e= 0.1)", "coef(m1)", "coef(m2)", fitAR1 = FALSE,fitAR2 = FALSE,TRUE,  num_data_points = num_data_points, ylabel, "Weibull")

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW 
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m2)",  fitAR1 = FALSE,fitAR2 = FALSE, TRUE,  num_data_points = num_data_points, ylabel, "Power")

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE,  fitAR1 = FALSE,fitAR2 = FALSE,num_data_points = num_data_points, ylabel, "Asymptotic")

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE,  fitAR1 = TRUE,fitAR2 = TRUE, num_data_points = num_data_points, ylabel, "Shepherd")

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)","coef(m1)", "coef(m2)", TRUE,fitAR1 = TRUE,fitAR2 = FALSE,  num_data_points = num_data_points, ylabel, "Hassell")
...
# Adjust for the fact that arcsin sqrt back transforms must be bounded
if (ARCSINSQRT)
{
  for (ii in 1:dim(modelpred)[1])
  {
    for (jj in 1:dim(modelpred)[2])
    {
      if (modelpred[ii,jj] > pi/2)
        modelpred[ii,jj] = pi/2
    }
  }
}
...
writepdfsandresults(output_directory_images, output_directory_files, output_name, 0)
